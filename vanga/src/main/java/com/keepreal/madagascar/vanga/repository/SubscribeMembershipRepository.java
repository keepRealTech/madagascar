...
public interface SubscribeMembershipRepository extends JpaRepository<SubscribeMembership, String> {
    ...
    @Query(value = "SELECT COUNT(*) FROM subscribe_membership WHERE island_id = ?1 AND expire_time < ?2 and is_deleted = FALSE", nativeQuery = true)
    Integer getMemberCountByIslandId(String islandId, long deadline);

    @Query(value = "SELECT COUNT(*) FROM subscribe_membership WHERE  membership_id = ?1 AND expire_time < ?2 and is_deleted = FALSE", nativeQuery = true)
